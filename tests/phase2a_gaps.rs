...
/// Multi-thread recv().wait() PoC test.
///
...
///
/// The test uses the "threads" mode of the C++ fixture which spawns audio worker
/// threads that call audio::process_buffer in a loop.
#[tokio::test(flavor = "multi_thread")]
async fn test_multithread_recv_wait_poc() {
    ...
    for _ in 0..20 {
        ...
        if !paused.is_empty() {
            ...
            if paused.len() >= 2 {
                ...
                let first_thread = *paused.keys().next().unwrap();
            ...
            // Even with 1 paused thread, the recv().wait() mechanism works
            let resume_result = sm
                .debug_continue_async(session_id, Some("continue".to_string()))
                .await;
            assert!(resume_result.is_ok());
            println!("✓ Resumed single paused thread, waiting for more...");
        }
    }
...
/// CModule + breakpoint coexistence test.
///
...
/// pauses correctly.
#[tokio::test(flavor = "multi_thread")]
async fn test_cmodule_breakpoint_coexistence() {
    let binary = cpp_target();
    let project_root = binary.parent().unwrap().to_str().unwrap();
    let (sm, _temp_dir) = create_session_manager();
