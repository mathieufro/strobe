...
async fn test_python_crashes(
    ...
    // Process may exit before Frida attaches — that's fine for crash tests,
    // pipe readers still capture stderr output
    match &spawn_result {
        Ok(pid) => eprintln!("Attached to crash process PID {}", pid),
        Err(e) => eprintln!("Process exited before attach (expected for crash): {}", e),
    }

    // Wait for crash output to arrive via pipe readers
    tokio::time::sleep(Duration::from_secs(2)).await;

    let events = sm
        .db()
        .query_events(session_id, |q| q.event_type(EventType::Stderr).limit(50))
        .unwrap();
    ...
    assert!(
        output.contains("RuntimeError") || output.contains("intentional crash"),
        "Should capture Python crash output in stderr, got: {}",
        output
    );
