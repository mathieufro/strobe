...
pub fn extract_functions_from_source(source: &str, file: &Path) -> crate::Result<FunctionTable> {
    ...
    let re_class =
        Regex::new(r"^\s*(?:@\S+\s*)*(?:export\s+)?(?:default\s+)?(?:abstract\s+)?class\s+(\w+)")
            .unwrap();
...
"#,
        ...
        let all = resolver
            .resolve_pattern("Calculator.*", dir.path())
            .unwrap();
        assert_eq!(all.len(), 2);
        ...
        assert_eq!(
            star.len(),
            1,
            "* matches only top-level functions (not Class.method)"
        );
        ...
        assert_eq!(
            dstar.len(),
            3,
            "** matches all functions including class methods"
        );
    }
...
"#,
    ...
    fn test_skips_excluded_dirs() {
        ...
        for skip in &["node_modules", ".git", ".next", "coverage"] {
            ...
            std::fs::write(
                dir.path().join(skip).join("index.js"),
...
"#;
    ...
    fn test_file_pattern() {
        ...
        let resolver = JsResolver::from_project(dir.path()).unwrap();
        let math_fns = resolver
            .resolve_pattern("@file:math.ts", dir.path())
            .unwrap();
        assert_eq!(math_fns.len(), 2);
        let util_fns = resolver
            .resolve_pattern("@file:utils.ts", dir.path())
            .unwrap();
    ...
    fn test_sourcemap_resolution() {
        ...
        if let Some((ts_file, ts_line)) = resolver.resolve_sourcemap(&js_path, 2, 0) {
            assert!(
                ts_file.to_string_lossy().ends_with("index.ts"),
                "should map to .ts file"
            );
