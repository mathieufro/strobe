            ...
            CoordinatorCommand::Spawn {
                ...
                let result = (|| -> Result<SpawnResult> {
                    ...
                    let pid = if is_interpreted {
                        ...
                        // resume, killing setInterval/recv in the agent.
                        // Workaround: spawn the process ourselves, then attach.
                        ...
                        // Register in output_registry for PID tracking (stop handler uses this)
                        ...
                        // Frida spawn for native binaries
                    ...
                    // -------------------------------------------------------
                    // Step 2: Attach Frida to the process (common path)
                    // -------------------------------------------------------
                    let t = std::time::Instant::now();
                    let frida_session = {
                        let mut last_err = String::new();
                        ...
                        attached.ok_or_else(|| {
                            ...
                            if !alive && is_interpreted {
                                tracing::error!("Process {} exited before Frida could attach (short-lived program?)", pid);
